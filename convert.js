...
// تابع تبدیل کد دو حرفی کشور (مثل DE) به ایموجی پرچم (مثل 🇩🇪)
...
// تابع استعلام واقعی GeoIP با استفاده از دستور curl درون گیت‌هاب اکشن
function getRealCountryFlag(address) {
    // اگر آدرس دامنه Cloudflare باشد
    if (address.includes('workers.dev') || address.includes('pages.dev')) {
        return '☁️';
    }

    try {
        // استفاده از curl برای استعلام واقعی لوکیشن IP از یک سرویس رایگان GeoIP
    ...
    return '🌍'; // پیش‌فرض اگر استعلام ناموفق بود
}
...
function parseVless(link) {
    try {
        ...
        // استعلام واقعی لوکیشن و دریافت پرچم
        console.log(`در حال بررسی لوکیشن واقعی برای: ${address}`);
        const flag = getRealCountryFlag(address);
        
        // پاکسازی پرچم‌های قدیمی از انتهای ریمارک برای جلوگیری از تکرار
...
// پردازش تمامی خطوط فایل و ساخت آرایه outbounds
const outboundsList = [];
let primaryRemarks = "Configs";

lines.forEach((line, index) => {
    if (line.startsWith('vless://')) {
        const config = parseVless(line);
        if (config) {
            if (index === 0) primaryRemarks = config.remarks;
            ...
            outboundsList.push({
                ...
                "tag": index === 0 ? "proxy" : `proxy-${index + 1}`
            });
        }
    }
});
...
// اضافه کردن Outbound های پیش‌فرض (dns, direct, block)
outboundsList.push(
    {
        "protocol": "dns",
